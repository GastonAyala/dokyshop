window.addEventListener('load', () => {
    ...
    const errImgSecondary = document.querySelector('.err-imgSecondary')
    ...
    title.addEventListener('blur', function (e) {
        ...
        switch (true) {
            ...
            case value.length < 5 || value.length > 100:
                invalid(errTitle, 'El título debe tener un mínimo de 5 caracteres', this)
                existTitleError = true;
                break;
            default:
                valid(errTitle, this)
    ...
    formCreate.addEventListener("submit", function (e) {
        ...
        switch (true) {
            case existImgPrima:
            case existImgSecon:
            case existTitleError:
            case existCateError:
            case existSubcaError:
            case existDescripError:
            case existPriceError:
            case existSaleError:
            case existQuanError:
            case existColorError:
                errFormGeneral.innerHTML = 'Hay errores en algunos campos';
                errFormGeneral.classList.add("alert", "alert-danger");
                fieldRequired.forEach((field) => (field.innerHTML = " *"));
                break;
            default:
                this.submit();
                break;
